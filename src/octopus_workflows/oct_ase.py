...
def parse_oct_structure_to_atoms(options: dict) -> ase.atoms.Atoms:
    ...
    lattice_parameters = [
        x * bohr_to_ang for x in options["LatticeParameters"]
    ]
    ...
    for entry in options["Coordinates"]:
        positions.append(
            np.array([r * bohr_to_ang for r in entry[1:]]) - origin
        )

    atoms = ase.atoms.Atoms(
        symbols=[entry[0] for entry in options["Coordinates"]],
        positions=positions,
        pbc=True,
    )
...
def ase_atoms_to_oct_structure(atoms: ase.atoms.Atoms, fractional=True) -> str:
    ...
    input_string += (
        " ".join(f"{constant} |" for constant in constants[0:3])[:-1] + "\n"
    )
    input_string += "%\n"
    ...
    if fractional:
        ...
        positions = (
            np.asarray(atoms.get_scaled_positions()) - fractional_origin
        )
    else:
        key = "Coordinates"
    ...
    for i in range(0, n_atoms):
        line = (
            f'"{species[i]}"{whitespace[i]} | '
            + " ".join(f"{r:.18e} |" for r in positions[i])[:-1]
        )
        input_string += line + "\n"
    input_string += "%\n"
